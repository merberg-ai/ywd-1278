...
[[ "${artifact_sha,,}" == "${expected_sha,,}" ]] || die "Firmware does not match the exact 0B-P1R1 qualified SHA256"
...
warn "This qualification will write the exact YWD-1278 P1R1 image to the STM32 main flash, read it back, then restore the exact verified stock image in the same run."
...
ok "Programmed YWD-1278 bytes match the exact 0B-P1R1 artifact"
...
step "$ywd_identity"
[[ "$ywd_identity" == "$expected_identity" ]] || die "YWD-1278 identity differs from the exact 0B-P1R1 identity"
