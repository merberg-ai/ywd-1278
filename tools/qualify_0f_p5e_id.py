#!/usr/bin/env python3
"""Guarded 0F-P5e one-shot physical station-ID acceptance harness.

Dry-run is the default and performs zero device or service I/O. Physical mode
temporarily runs the P5e source graph, submits one manual ``ID`` command through
the actual classic Telnet console, observes exactly one dispatch, and restores
the unchanged persistent no-TX appliance.
"""
...
EXPECTED_HOST_COMMIT = "79c249167902256399070482d773b39bda96fdea"
TEMP_ROOT = Path("/run/ywd-1278-0f-p5e")
TEMP_CONFIG = TEMP_ROOT / "config.toml"
TEMP_LOG = TEMP_ROOT / "daemon.log"
TEMP_KISS_PORT = 18201
TEMP_CONSOLE_PORT = 18210
TEMP_PTY = "/run/ywd-1278-0f-p5e/tnc"

AUTHORIZATION_TOKEN = "0F-P5E-ID-145050-ONE"
ARM_PHRASE = "TRANSMIT-0F-P5E-ID-ONE"
EXTERNAL_PHRASE = "EXTERNAL-ID-DECODE-MATCH-ONE"
DESTINATION = "ID"
INFORMATION_PREFIX = "YWD-1278 ID "
...
FROZEN_BLOBS = {
    ...
    "src/ywd1278/daemon.py": "9a1c5ec38317c09a7d960e844dbca3e71efc33a2",
...
def validate_frozen_capability_blobs() -> None:
    for relative, expected in FROZEN_BLOBS.items():
        ...
        if actual != expected:
            raise RuntimeError(f"frozen P5e capability mismatch: {relative} actual={actual}")
...
def wait_for_one_dispatch(console: socket.socket) -> str:
    ...
    while time.monotonic() < deadline:
        ...
        if count > 1:
            raise RuntimeError("more than one P5e ID dispatch occurred")
        if count == 1:
            stage_i.assert_single_shot_status(last, require_dispatched=True)
            return last
        time.sleep(0.1)
    raise RuntimeError(f"timed out waiting for one P5e ID dispatch: {last!r}")
...
def recv_post_tx_packet(kiss: socket.socket, *, tx_source: str, timeout: float) -> tuple[bytes, str]:
    ...
    while time.monotonic() < deadline:
        try:
            ...
            continue
        if not chunk:
            raise RuntimeError("KISS observer closed while waiting for post-ID RX")
        for message in decoder.feed(chunk):
            ...
            if source == tx_source and info == f"{INFORMATION_PREFIX}{tx_source}".encode("ascii"):
                continue
            return message.frame, source
    raise RuntimeError("timed out waiting for a non-qualification post-ID RX packet")


def expected_external_decode(source: str = "KJ6YWD-10") -> str:
    return f"{source}>{DESTINATION}:{INFORMATION_PREFIX}{source}"


def print_plan(source: str = "KJ6YWD-10") -> None:
    print("===== YWD-1278 0F-P5E ONE-SHOT ID ACCEPTANCE =====")
    print(f"HOST_CHECKPOINT={EXPECTED_HOST_COMMIT}")
    print(f"SOURCE={source}")
    print(f"DESTINATION={DESTINATION}")
    print("PATH=DIRECT")
    print(f"INFORMATION={INFORMATION_PREFIX}{source}")
    print(f"EXPECTED_EXTERNAL_DECODE={expected_external_decode(source)}")
    print("TX_FREQUENCY_HZ=145050000")
    print("TX_POWER=200")
    print("MANUAL_ID_COMMANDS_MAX=1")
    print("INTERNAL_TX_DISPATCHES_MAX=1")
    print("AUTOMATIC_TX_RETRY=NO")
    print("BEACON_SCHEDULER_USED=NO")
...
def main() -> int:
    ap = argparse.ArgumentParser(description="0F-P5e guarded one-shot physical station ID")
    ...
    if not args.transmit:
        print("YWD1278_0F_P5E_DRY_RUN=PASS")
        print("SERVICE_MUTATED=NO")
        print("MODEM_UART_OPENED=NO")
        print("ID_COMMAND_SUBMITTED=NO")
        print("RF_TRANSMITTED=NO")
        return 0

    if os.geteuid() != 0:
        raise SystemExit("[FAIL] physical P5e requires root")
    ...
    if ancestry.returncode != 0:
        raise SystemExit("[FAIL] source checkout does not descend from the P5e host checkpoint")
    ...
    print("===== P5E PRE-ARM QUALIFIED STATE =====")
    print(f"PERSISTENT_CONFIG_SHA256={original_hash}")
    print("PERSISTENT_TX_ENABLED=NO")
    print("PERSISTENT_BEACON_ENABLED=NO")
    typed = input(f"Type exactly {ARM_PHRASE} to submit ONE manual RF ID: ").strip()
    if typed != ARM_PHRASE:
        raise SystemExit("[FAIL] P5e interactive arm phrase did not match")
    ...
    try:
        ...
        if TEMP_ROOT.exists():
            raise RuntimeError(f"stale P5e runtime directory exists: {TEMP_ROOT}")
        ...
        if not packet_cfg.tx_enabled or packet_cfg.tx_power != stage_i.TX_POWER or packet_cfg.frequency_hz != stage_i.EXPECTED_FREQUENCY_HZ:
            raise RuntimeError("temporary P5e TX profile failed validation")
        if not classic_cfg.configured or str(classic_cfg.source) != source:
            raise RuntimeError("temporary P5e station identity failed validation")
        ...
        if daemon.poll() is not None:
            raise RuntimeError(f"temporary P5e daemon exited early rc={daemon.returncode}")
        ...
        with socket.create_connection(("127.0.0.1", TEMP_KISS_PORT), timeout=3.0) as kiss, socket.create_connection(("127.0.0.1", TEMP_CONSOLE_PORT), timeout=3.0) as console:
            ...
            reply = console_command(console, "ID")
            if "ID QUEUED REQUEST=" not in reply or "DEST=ID VIA=DIRECT" not in reply:
                raise RuntimeError(f"ID submission failed: {reply!r}")
            print("ID_COMMAND_SUBMITTED=YES")

            wait_for_one_dispatch(console)
            tx_dispatched = True
            print("P5E_AUTHORIZATION_CONSUMED=YES")
            ...
            print("NO_AUTOMATIC_RETRY_AFTER_SETTLE=PASS")
            ...
            if external != EXTERNAL_PHRASE:
                raise RuntimeError("independent exact ID decode not confirmed; no retry permitted")
        ...
        if pty_leaked and cleanup_error is None:
            cleanup_error = RuntimeError("temporary P5e PTY leaked")
        if cleanup_error is not None:
            raise cleanup_error

    if not tx_dispatched:
        raise SystemExit("[FAIL] P5e ended without its one ID dispatch")
    if stage_i._systemctl_state("is-enabled") != "enabled" or stage_i._systemctl_state("is-active") != "active":
    ...
    if hashlib.sha256(restored).hexdigest() != original_hash:
        raise SystemExit("[FAIL] persistent config changed during P5e")
    stage_i.validate_persistent_config(tomllib.loads(restored.decode("utf-8")))

    print("YWD1278_0F_P5E_ONE_SHOT_ID=PASS")
    print("MANUAL_ID_COMMANDS=1")
