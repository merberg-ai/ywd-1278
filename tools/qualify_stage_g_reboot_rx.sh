...
while (($#)); do
  case "$1" in
    ...
    -h|--help)
      echo "Usage: sudo bash tools/qualify_stage_g_reboot_rx.sh [--timeout 120] [--firmware FILE]"
      exit 0
      ;;
...
for path in "$VENV/bin/python" "$SOURCE_ROOT/installer/hardware-detect.sh" "$SOURCE_ROOT/firmware/product-ax25r4.json" "$SOURCE_ROOT/tools/qualify_stage_g_live_rx.py" "$SCRIPT_DIR/qualify_stage_g_reboot_live_rx.py" "$CONFIG" "$ELIGIBILITY" "$MARKER" /opt/ywd-1278/installed-commit /proc/sys/kernel/random/boot_id "$FIRMWARE"; do
...
assert d['schema']==1 and d['stage']=='G-reboot'
print(d['boot_id_before'])
print(d['installed_commit'])
print(d['main_pid_before'])
print(d['kiss_port'])
print(d['console_port'])
print(d['pty'])
PY
)
...
eligibility="$($VENV/bin/python -m ywd1278.install.firmware_trust \
  --profile "$SOURCE_ROOT/firmware/product-ax25r4.json" check-eligibility \
  --config "$CONFIG" --firmware "$FIRMWARE" --record "$ELIGIBILITY")"
printf '%s\n' "$eligibility"
grep -q '^SERVICE_ELIGIBLE=YES$' <<<"$eligibility" || { echo "[FAIL] Stage-F eligibility did not survive/revalidate after reboot" >&2; exit 5; }

# This is the auto-start proof: all assertions below happen before this script
# performs any service start/restart action.
...
echo "YWD1278_STAGE_G_REBOOT_AUTOSTART=PASS"
...
if fuser "$DEVICE" >/dev/null 2>&1; then
  echo "[FAIL] UART ownership leaked after post-reboot stop" >&2
  fuser -v "$DEVICE" >&2 || true
  exit 7
fi

detect="$(YWD1278_SOURCE_ROOT="$SOURCE_ROOT" bash "$SOURCE_ROOT/installer/hardware-detect.sh" --device "$DEVICE" --config "$CONFIG")"
printf '%s\n' "$detect"
...
for _ in {1..50}; do
  [[ "$(systemctl is-active "$SERVICE" 2>/dev/null || true)" == active && -e "$pty" ]] && break
  sleep 0.1
done
...
[[ "$restart_pid" =~ ^[1-9][0-9]*$ ]] || { echo "[FAIL] restarted service has no MainPID" >&2; exit 8; }
...
for _ in range(30):
    try:
        with socket.create_connection(('127.0.0.1',port),timeout=1):
            pass
        print(f'POST_IDENTITY_RESTART_LOOPBACK_PORT_{port}=PASS')
        raise SystemExit(0)
...
echo "===== STAGE G REBOOT QUALIFICATION COMPLETE ====="
echo "YWD1278_STAGE_G_REBOOT_RX=PASS"
echo "BOOT_ID_CHANGED=YES"
