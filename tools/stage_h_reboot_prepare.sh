...
[[ "$installed_commit" == "$EXPECTED_INSTALLED_COMMIT" ]] || {
  echo "[FAIL] installed commit is not the qualified Stage-H fresh-OS candidate" >&2
...
d={'schema':1,'stage':'H-reboot','prepared_unix_ns':time.time_ns(),'boot_id_before':boot_id,'installed_commit':commit,'service_enabled_before':True,'service_active_before':True,'main_pid_before':int(pid),'frequency_hz':145050000,'tx_enabled':False,'automatic_flash_enabled':False,'kiss_host':'127.0.0.1','kiss_port':int(kiss_port),'console_host':'127.0.0.1','console_port':int(console_port),'pty':'/run/ywd-1278/tnc'}
with open(path,'w',encoding='utf-8') as f:
    json.dump(d,f,indent=2,sort_keys=True); f.write('\n')
PY
chmod 0600 "$MARKER"

echo "YWD1278_STAGE_H_REBOOT_PREPARE=PASS"
