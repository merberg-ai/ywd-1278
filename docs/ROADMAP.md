...
- [ ] 0B-P13b guarded known-packet YWD TX on 145.050 MHz with independent external decoder/receiver proof — first physical one-shot **passed all internal RF evidence** (`1` submission, keyups `0->1`, generated samples `0->12048`, idle after completion, UART released) but external decode was not confirmed, so P13b remains incomplete. P13b-R1 is now staged/CI-gated as exactly three fixed packets `VERIFY 1/3`, `2/3`, `3/3`, each 721 selectors / 11536 samples, with fixed 5.0 s gaps; aggregate expected delta is exactly 3 keyups / 34608 samples, no automatic retry, and ordinary KISS/product TX remains disconnected
