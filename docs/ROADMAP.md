...
- [ ] channel-busy/recent-RX detection (0C-P2) — the exact self-contained AX25R4 RSSI firmware is now **physically activated and raw-telemetry qualified** on the target: 59892-byte artifact/readback SHA256 `b06fcbf0baa36e865198091cee27c66e1624ef08117ee685253a7a5613c7c616`, exact AX25R4 runtime identity verified, firmware intentionally left installed, 401 RSSI samples over 20 s at 145.050 MHz, 48021 packed RX bytes drained, zero FIFO drops, RF keyups `0->0`, RF TX samples `0->0`, UART released. The first capture contained three coherent low-value events and a large separation between event/transition values and normal values; no carrier threshold or hysteresis is selected yet. A receive-only packet-correlated characterization gate is staged next to prove polarity and guard-gap margin with real FCS-valid AX.25 before any busy/clear classifier is enabled. KISS/product TX remains disconnected
- [ ] TXDELAY
- [ ] PERSIST
- [ ] SLOTTIME
