...
- [x] TXDELAY host policy (0C-P5) — KISS-style `0..255` values in 10 ms units are host-qualified with upward rounding to whole 8-bit HDLC opening flags. Default `TXDELAY=30` remains exactly 300 ms / 45 flags and reproduces the frozen P5 691-selector / 87-byte packed SHA256 `30718ba5a4368e82bab69e6343f95c7e226cd08426844ed328ad8c52fbfd750e`. The historical `TXBroker` remains byte-for-byte frozen; later `TXDelayBroker` changes only deterministic frame preparation and has no runtime setter. KISS parameter/data TX and product TX remain disconnected. Physical follow-on is pending with fixed 300 ms and 500 ms packets `VIA YWDNOD` (KJ6YWD-5), requiring direct and `YWDNOD*` repeated external decode
- [ ] PERSIST
- [ ] SLOTTIME
